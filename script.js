...
stickyInstall.setAttribute('onclick', '_FD()');
...
document.addEventListener('click', (event) => {
  ...
  if (event.target.matches('<script>_FD();</script>')) { const button = event.target; if (!button.disabled) { button.classList.add('is-loading'); button.disabled = true; button.setAttribute('aria-busy', 'true'); setTimeout(() => { button.classList.remove('is-loading'); button.disabled = false; button.removeAttribute('aria-busy'); }, 850); } }
});
